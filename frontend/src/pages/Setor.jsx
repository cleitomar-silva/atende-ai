...
import { Card, PageHeader, EmptyState, Drawer, Field, inputClass, StatusBadge, useForm, Spinner } from '../components/dashboard/ui.jsx'
...
export default function Setor() {
  ...
  const openEdit = (s) => {
    setEditing(s)
    setValues({ name: s.name, is_active: Boolean(s.is_active) })
    setError('')
    setOpen(true)
  }
  ...
  const save = async (e) => {
    ...
    try {
      if (editing) {
        await api(`/sectors/${editing.id}`, { method: 'PUT', body: { ...values, is_active: Boolean(values.is_active) } })
      } else {
        await api('/sectors', { method: 'POST', body: { name: values.name } })
      }
      setOpen(false)
      load()
      toast.success(editing ? 'Setor atualizado com sucesso.' : 'Setor cadastrado com sucesso.')
    } catch (err) {
  ...
  const remove = async (s) => {
    const ok = await confirm({
      title: 'Excluir setor',
      message: `Excluir o setor "${s.name}"?`,
      confirmText: 'Excluir',
      danger: true,
    })
    if (!ok) return
  ...
  return (
    <div>
      <PageShell>
        <PageHeader title="Setores" subtitle="Gerencie os setores da sua empresa." />
        <Card>
          ...
          {loading ? (
            ...
            <EmptyState message="Nenhum setor cadastrado ainda." />
          ) : (
              ...
              <table className="w-full zebra-table">
                <thead>
                  <tr className="text-label-md text-on-surface-variant uppercase">
                    <th className="px-lg py-sm text-left">Nome do setor</th>
      ...
      <Drawer
        ...
        footer={
          <>
            ...
            <button type="submit" form="setor-form" disabled={saving} className="px-md py-sm rounded-lg bg-primary text-on-primary font-label-md hover:bg-surface-tint disabled:opacity-60 transition-all">
              {saving ? 'Gravando…' : 'Gravar'}
            </button>
          </>
        }
      >
        <form id="setor-form" onSubmit={save} className="space-y-lg">
          {error && <div className="px-md py-sm bg-error-container text-on-error-container rounded-lg font-body-md">{error}</div>}
          <div className="space-y-base">
            <label className="font-label-md text-label-md text-on-surface-variant">Nome do setor <span className="text-error">*</span></label>
            <input className={inputClass} name="name" value={values.name} onChange={handleChange} required placeholder="Ex.: TI" />
          </div>
          {editing && (
